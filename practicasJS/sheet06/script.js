let pagina = 1;
const filter = document.getElementsByClassName("searchbar")[0];
obtenerPokemon();
...
filter.addEventListener("keyup", async (event) => {
  
  if (filter.value.trim() != null) {
    try {
      const filterRequest = await fetch(
		//no se puede hacer la request parece
        `https://pokeapi.co/api/v2/pokemon/${filter.textContent.trim()}`
      );
      if (!filterRequest.ok)
        throw new Error("No se han encontrado pokemon", error);
      const filteredpokemon = await filterRequest.json();
	  console.log(filteredpokemon)
      obtenerDatos(filteredpokemon);
    } catch {}
  } else {
    obtenerPokemon();
  }
